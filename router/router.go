...
func SetupRouter() *gin.Engine {
	...
	userGroup.GET("/:id/points", handler.GetUserPoints)
	userGroup.POST("/:id/points/add", handler.AddUserPoints)
	userGroup.POST("/:id/points/deduct", handler.DeductUserPoints)
	...
	cardGroup.PUT("/:id/tags", handler.ReplaceCardTags)
	cardGroup.DELETE("/:id/tags/:tag_id", handler.DeleteTagFromCard)

	tagGroup := apiv1.Group("/tags")
	tagGroup.Use(middleware.Auth)
	tagGroup.POST("", handler.CreateTag)
	tagGroup.GET("/:id", handler.GetTagById)
	tagGroup.POST("/query", handler.QueryTag)
	tagGroup.PUT("/:id", handler.UpdateTag)
	tagGroup.DELETE("/:id", handler.DeleteTag)

	cardTagGroup := apiv1.Group("/card-tags")
	cardTagGroup.Use(middleware.Auth)
	cardTagGroup.POST("/query", handler.QueryCardTags)
	...
	ragGroup2.POST("", handler.RagVectorizeTask)
